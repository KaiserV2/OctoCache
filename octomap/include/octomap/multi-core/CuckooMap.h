...
public:
    ...
    struct Entry{
        ...
        Entry(Bucket* _bucket = nullptr, uint32_t _position = -1):
                bucket(_bucket), position(_position){};
    };
    ...
    bool Insert(KEY_TYPE key, VALUE_TYPE value){
        ...
        for(uint32_t kick_num = 0;kick_num < MAX_KICK;++kick_num){
            uint32_t slot = rng() % SLOT_PER_BUCKET;
    ...
    Entry Lookup(KEY_TYPE key){
        for(uint32_t i = 0;i < ARRAY_NUM;++i){
            ...
            for(uint32_t slot = 0;slot < SLOT_PER_BUCKET;++slot){
                if(bitmaps[i]->Get(start + slot) &&
                   buckets[i][pos].keys[slot] == key){
                    return Entry((&buckets[i][pos]), slot);
                }
            }
        }
